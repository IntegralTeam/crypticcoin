...
class dPoS_BaseTest(BitcoinTestFramework):
    ...
    def create_transaction_withInput(self, node_idx, nodeSend_idx, input, to_address, amount, instantly):
        node = self.nodes[node_idx]
        outputs = { to_address : int((amount - amount * 0.000001) * 100000) / 100000.0 }
        inputs = [{'txid': input['txid'], 'vout': input["vout"], 'scriptPubKey' : input['scriptPubKey'], 'amount' : input["amount"]}]
        rawtx = node.createrawtransaction(inputs, outputs, 0, node.getblockcount() + 21, instantly)
        fundtx = node.fundrawtransaction(rawtx)
        sigtx = node.signrawtransaction(fundtx["hex"])
        return self.nodes[nodeSend_idx].sendrawtransaction(sigtx["hex"])
