    ...
    try {
        while (true) {
            ...
            while (true) {
                ...
                if (solver == "tromp") {
                    // @maxb Мне кажется, в оригинальном майнере здесь могла быть ошибка.
                    // Потому что если блок не прошел валидацию (а прошёл весь цикл), то тоже окей.
                    // Либо там сильно напутано. По-разному работает валидация в майнере и rpc:generate
                    // Рефакторинг был абсолютно формальным, с сохранением предыдущей логики
