...
TEST(dPoS_storm, RealisticStorm)
{
    ...
    ASSERT_GE(committedNum, 10u - 2 * 4u);
    ASSERT_LE(committedNum, 10u - 4u);
}

// Like RealisticStorm, but with 6 nodes and 50 iterations
TEST(dPoS_storm, ExtraLongStorm)
{
    for (unsigned int seed = 0; seed < 50u; seed++) {
        StormTestSuit suit{};
        suit.seed = seed;
